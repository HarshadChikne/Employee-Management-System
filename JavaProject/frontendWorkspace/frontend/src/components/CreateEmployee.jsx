...
function CreateEmployee() {
    ...
    const validate=()=>{
        ...
        if(!employees.doj)
        {
            formErrors.doj="DOJ is mandatory";
            isValid=false;
        }
    ...
    const handleSubmit=(e)=>{
        ...
        if(validate())
        {
            const formatDate=dateFormat(employees.doj)
            const employeeData=
            {
                ...employees,doj:formatDate
            }

            EmployeeService.addEmployee(employeeData).then(() => {
                toast.success("Employee Joined Successfully")
                navigate("/");
                
              })
              .catch(() =>toast.error("Joining Failed"));
              }
        }
      ...
      <div className='container pt-3'>
        <div className='card w-50 offset-3 p-3'>
            ...
            <form action="">
                ...
                <label>DOJ:</label>
                <input type="text" id='doj' name='doj' className='form-control' 
                value={employees.doj}
                onChange={handleChange}/>
